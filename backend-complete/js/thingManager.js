...
*/
  ...
  this.sortByPrototype = function( things_ ) {
    var sortedThings = [];
    for( var i = 0; i < things_.length; i++ ) {
      var thing = things_[i];
      var exists = false;
      var j = 0;
      for( j = 0; j < sortedThings.length; j++ ) {
        if( sortedThings[j].prototype.id === thing.prototype ) {
          exists = true;
          break;
        }
      }
      if( exists ) {
        sortedThings[ j ].things.push(thing);
      } else {
        var proto = PrototypeManager.getPrototype( thing.prototype );
        var sortedThingsEntry = {
          'prototype' : proto,
          'things' : [ thing ]
        };
        sortedThings.push( sortedThingsEntry );
      }

/*
      if( !sortedThings.hasOwnProperty( proto.id ) ) {
        sortedThings[ proto.id ] = [];
      }
*/
    }
    return sortedThings;
